...
import { gameState, cheats } from "../global";
  ...
  return (
    <section className="mt-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-2 lg:gap-3 mt-10 mb-5">
        {
          whispers && whispers
            .filter(w => gameState.unlockedWhispers.includes(w.id) || cheats.unlockAllWhispers)
            .map(w => {
              const Icon = w.icon;
              return (
                <div key={w.id} className="grid grid-cols-7 p-5 bg-green-400/30 dark:bg-black/30 shadow-lg rounded-xl mb-3 md:mb-0">
                  ...
                  <a
                    className={`${(w.id==='whisper_buy_picture' && !context.hasPhoto || w.id==='whisper_buy_picture' && context.purchasedPhoto) ? "disabled bg-gray-800 cursor-not-allowed hover:bg-gray-800" : "cursor-pointer hover:bg-cyan-600 dark:hover:bg-cyan-950 hover:text-white" } col-span-3 rounded-sm p-2 bg-cyan-300 text-black block text-center w-full text-xs md:text-lg select-none`}
                  ...
                  <span className="col-span-2 inline my-auto ml-auto mr-2"><WongoPoint /></span>
                  <span className="col-span-1 inline my-auto font-bold select-none pointer-events-none">x{w.cost}</span>
                </div>
              )
            })
        }
